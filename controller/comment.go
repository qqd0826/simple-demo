...
func CommentAction(c *gin.Context) {
	...
	if res := db.DB.Where("username = ?", token).First(&user); res.Error == nil {
		...
		if actionType == "1" {
			...
			// 插入数据
			comment := model.Comment{User: user, VideoId: int64(videoId), Content: text, CreateDate: strconv.Itoa(int(time.Now().Unix()))}
			...
			db.DB.Where("id = ?", commentId).Delete(&model.Comment{})
		}
